...
import re  # Regex helpers for extracting timestamp embeds.
...
from django.utils import timezone  # Timezone helpers for safe datetimes.
from django.utils.dateparse import parse_datetime  # Parse ISO datetime strings.
...
# Match the timestamp embed attribute generated by the notes editor.
_NOTE_TIMESTAMP_ATTR_RE = re.compile(
    r"data-date_time_now\s*=\s*['\"]([^'\"]+)['\"]",
    re.IGNORECASE,
)


# Extract the first embedded timestamp datetime from saved HTML.
def _extract_first_note_timestamp(content_html):
    # Return early when HTML is empty.
    if not content_html:
        return None
    # Search for the first timestamp attribute in the HTML.
    match = _NOTE_TIMESTAMP_ATTR_RE.search(str(content_html))
    # Exit when no timestamp is present.
    if not match:
        return None
    # Read the raw ISO datetime string.
    raw_value = (match.group(1) or "").strip()
    # Exit when the value is empty.
    if not raw_value:
        return None
    # Normalize trailing Z to an explicit UTC offset for Django parsing.
    ...
    # Normalize to UTC for storage consistency.
...
def _serialize_note(note):
    ...
    return {
        ...
        "date_time_now": note.date_time_now.isoformat() if note.date_time_now else None,
...
class NoteListCreateView(LoginRequiredMixin, View):
    ...
    def post(self, request):
        ...
        # Extract the first timestamp embed (if present).
        extracted_timestamp = _extract_first_note_timestamp(content_html)
        ...
        note = Note.objects.create(
            ...
            date_time_now=extracted_timestamp,  # Persist first embedded timestamp.
        )
...
class NoteDetailView(LoginRequiredMixin, View):
    ...
    def post(self, request, note_id):
        ...
        if content_html is not None:
            ...
            # Persist the first embedded timestamp once (immutable thereafter).
            if note.date_time_now is None:
                extracted_timestamp = _extract_first_note_timestamp(note.content_html)
                if extracted_timestamp is not None:
                    note.date_time_now = extracted_timestamp
